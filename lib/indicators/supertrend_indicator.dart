...
class SupertrendIndicator {
  /// Per-bar Supertrend direction. Null until ATR has enough bars.
  static List<bool?> bullSeries(
    List<CandleData> candles,
    SupertrendFilterParams params,
  ) {
    final n = candles.length;
    final out = List<bool?>.filled(n, null);
    final atrValues = IndicatorUtils.atr(candles, params.period);
    if (atrValues.length < 2) return out;

    final startIdx = params.period;
    final m = atrValues.length;
    final upperBand = List<double>.filled(m, 0);
    final lowerBand = List<double>.filled(m, 0);
    final isBull = List<bool>.filled(m, true);
