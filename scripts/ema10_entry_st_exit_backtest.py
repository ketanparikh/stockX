"""
EMA 10 Cross entry, Supertrend SELL exit — long backtest.

Entry:  EMA 10 Cross BUY (close > 10/200, 10 above 30 & 48, completing
        up-cross within lookback). Supertrend is not required to enter.
        Enter next open.

Exit:   Supertrend SELL. Exit next open.
...
OUTPUT_DIR = SCRIPT_DIR / "backtest_output" / "ema10_entry_st_exit_run"
...
def simulate_long(
    ...
    ema_entry, _ema_exit = entry_exit_series(c, lookback)
    ...
    while i < n - 2:
        ...
        for j in range(entry_i, n - 1):
            ...
            if np.isfinite(px) and px > 0:
                exit_i, exit_px, reason = nxt, px, "Supertrend SELL"
            else:
                exit_i, exit_px, reason = j, float(c[j]), "Supertrend SELL"
            break
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="EMA 10 Cross entry + Supertrend SELL exit NSE backtest"
    )
    ...
    print(
        f"EMA 10 entry + ST SELL exit | symbols={len(symbols)} "
    ...
    trades_path = out / "ema10_entry_st_exit_trades.csv"
    sum_path = out / "ema10_entry_st_exit_summary.txt"
    ...
    if sized.empty:
        ...
        summary = (
            f"EMA 10 Cross entry + Supertrend SELL exit — LONG\n"
            f"Universe: {len(symbols)} ({processed} with data) | period={period}\n"
            f"Entry: EMA 10 Cross BUY (no Supertrend required), next open\n"
            f"Exit: Supertrend SELL, next open | end of data\n"
